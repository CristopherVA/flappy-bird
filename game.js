...
const bird = {
  ...
  update: function () {
    ...
    if (state.current == state.getReady) {
      ...
      if (this.y + this.h / 2 >= HEIGTH - fg.h) {
        this.y = HEIGTH - fg.h - this.h / 2;
        if (state.current == state.game) {
          state.current = state.over
          DIE.play()
        }
      }
      // IF THE SPEED S GREATER THAN THE JUMP MEANS THE BIRD IS FALLING DOWN
...
const score = {
  ...
  draw: function () {
    ...
    if (state.current == state.game) {
      ...
      context.fillText(this.value, WIDTH / 2, 50);
      context.strokeText(this.value, WIDTH / 2, 50);
